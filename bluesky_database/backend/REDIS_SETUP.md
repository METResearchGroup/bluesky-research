...
## Security Posture (MET-27)

- Bind host exposure to `127.0.0.1` (localhost) via Docker Compose port mapping to restrict access to the local machine.
- Require authentication using `--requirepass` sourced from a Docker secret (`/run/secrets/redis_password`); never hardcode credentials in configs or scripts.
- Keep Redis internal-only by removing the `ports:` mapping if external host access is not needed; other services in the Docker network will still connect.
- For inter-container connectivity, Redis binds to `0.0.0.0` inside the container while host exposure remains restricted by the localhost mapping.
- Store secrets in `bluesky_database/backend/secrets/` with restrictive permissions (`chmod 600`) and do not commit them to source control.
...
- Network binding is set to `127.0.0.1` to illustrate secure defaults, and MET-27-required settings are included for local development:

  ```conf
  bind 127.0.0.1
  
  # MET-27: Required Redis settings for buffer capacity testing
  maxmemory 2gb
...
- Authentication is not hardcoded in the file. Instead, it is supplied at runtime via Docker secrets. The config includes commented placeholders and reiterates the MET-27 settings for completeness:

  ```conf
  # requirepass <password-from-docker-secret>
  # masterauth <password-from-docker-secret>
  
  # MET-27: Required Redis settings (documented here; kept active above)
  # maxmemory 2gb
  # maxmemory-policy allkeys-lru
  # appendonly yes
  # appendfsync everysec
...
Redis service snippet with non-destructive healthcheck using `redis-cli ping` and password from the secret:
...
services:
  redis:
    ...
    command: [
      "sh", "-c",
      "redis-server /usr/local/etc/redis/redis.conf --bind 0.0.0.0 --requirepass \"$(cat /run/secrets/redis_password)\""
    ]
    secrets:
      - redis_password
    healthcheck:
...
Add a non-destructive healthcheck that pings Redis using the password from the secret and verifies `PONG`:
