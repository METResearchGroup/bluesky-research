"""Model for the intergroup LLM service."""
...
from services.ml_inference.models import (
    LLMIntergroupLabelModel,
    LLMIntergroupLabelsModel,
    IntergroupLabelsModel,
)
...
def generate_prompt(posts: list[dict]) -> str:
    """Generates a structured prompt for LLM-based intergroup classification.
    ...
    Behavior:
        ...
        2. Joins all post texts into a single string and interpolates them into a multi-line template that instructs the LLM on how to classify posts as "intergroup" or "not intergroup".
    """
    enumerated_texts = ""
    ...
    prompt = f"""
You are a classifier that predicts whether a post has intergroup content or not. 
Intergroup content refers to posts that discuss, reference, or make comparisons 
between different social groups (e.g., political parties, ethnic groups, religious 
groups, social classes, etc.). This includes content that emphasizes "us vs them" 
distinctions, group-based conflicts, or group-based stereotypes.

Please classify the following text as "intergroup" or "not intergroup".
...
def parse_llm_result(
    ...
    Returns:
        list[LLMIntergroupLabelModel]: A list of label model objects parsed from the JSON string.

    Behavior:
        - Deserializes the JSON string into a LLMIntergroupLabelsModel object.
    ...
    """
    json_result_obj: LLMIntergroupLabelsModel = LLMIntergroupLabelsModel(
        **json.loads(json_result)
    )
    label_models: list[LLMIntergroupLabelModel] = json_result_obj.labels
...
def process_intergroup_batch(posts: list[dict]) -> list[dict]:
    ...
    Returns:
        list[dict]: A list of result dictionaries from the LLM classification for each post; if a mini-batch fails, corresponding entries are None.
    ...
    Behavior:
        ...
        5. Logs errors and replaces results with None if parsing fails.
    ...
    if len(json_results) != len(minibatches):
        logger.warning(
            f"Number of results ({len(json_results)}) does not match number of minibatches ({len(minibatches)}). Need to retry."
        )
        return [None] * len(posts)
    results: list[dict] = []
    for json_result, minibatch in zip(json_results, minibatches):
        try:
            parsed_llm_results: list[LLMIntergroupLabelModel] = parse_llm_result(
...
def process_intergroup_batch_with_retries(
    posts: list[dict],
    max_retries: int = 4,
    ...
    final_results = process_intergroup_batch(posts)
    ...
    while posts_to_retry and retries < max_retries:
        ...
        results = process_intergroup_batch(posts_to_retry)
...
def create_labels(posts: list[dict], responses: list[dict]) -> list[dict]:
    ...
    Returns:
        list[dict]: A list of dictionaries representing the labeled post information, which includes:
            ...
            - is_intergroup: The classification result for intergroup content.
    ...
    for post, response_obj in zip(posts, responses):
        label = IntergroupLabelsModel(
            ...
            is_intergroup=response_obj.get("is_intergroup", None)
            if response_obj
            else None,
        )
...
def batch_classify_posts(
    ...
    Behavior:
        ...
        2. For each batch:
            ...
            - Processes the batch with process_intergroup_batch_with_retries to obtain raw responses.
    ...
    for i, batch in enumerate(batches):
        ...
        responses: list[dict] = process_intergroup_batch_with_retries(posts=batch)
