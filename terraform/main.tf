    ...
    Statement = [
      ...
      # Add Secrets Manager policy
      {
        ...
        Resource = [
          ...
          "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.current.account_id}:secret:feed-api-default-test-token-xvJowB"
        ]
      },
      # Add DynamoDB policy
      {
...
resource "aws_glue_catalog_table" "archive_fetch_posts_used_in_feeds" {
  ...
  storage_descriptor {
    location      = "s3://${var.s3_root_bucket_name}/bluesky_research/2024_nature_paper_study_data/fetch_posts_used_in_feeds/cache/"
    input_format  = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
    output_format = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat"
