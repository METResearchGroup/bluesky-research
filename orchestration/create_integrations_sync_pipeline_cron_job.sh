...
SBATCH_CRON_JOB="$CRON_EXPRESSION cd $DIR && sbatch submit_integrations_sync_pipeline_job.sh"

# Add the cron job to the current user's crontab
(crontab -l 2>/dev/null; echo "$SBATCH_CRON_JOB") | crontab -

echo "Cron job created to run sbatch submit_integrations_sync_pipeline_job.sh once every 4 days at 8am UTC."
