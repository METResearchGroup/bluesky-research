...
class StorageManager:
    ...
    def create_scratch_path_for_job(self, job_id: str) -> str:
        """Creates a scratch path for a job."""
        scratch_path = os.path.join(self.root_scratch_dir, job_id)
        return scratch_path
    ...
    def export_batches_to_scratch(
        ...
        the codebase.
        """
        job_scratch_path = self.get_scratch_path_for_job(job_name, job_id)
        self.create_directory(job_scratch_path)

        logger.info(
            f"Exporting {len(batches)} batches to scratch directory for job {job_id} at path {job_scratch_path}."
        )
        for task_state, batch in zip(task_states, batches):
            task_scratch_path = self.get_scratch_path_for_task(
                job_scratch_path, task_state.id
            )
        ...
        logger.info(
            f"Exported all batches to scratch directory for job {job_id} at path {job_scratch_path}."
        )
