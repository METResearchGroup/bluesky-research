...
from services.add_context.current_events_enrichment.bsky_news_orgs import (
    bsky_did_to_news_org_name,
)  # noqa
from services.add_context.current_events_enrichment.newsapi_context import (
    url_is_to_news_domain,
)  # noqa
...
def process_record_context(record_uri: str) -> RecordContextModel:
    ...
    if embed_dict:
        if embed_dict["has_image"]:
            embed_image_alt_text: ImagesContextModel = process_images_embed(embed_dict)
    return RecordContextModel(text=text, embed_image_alt_text=embed_image_alt_text)
...
def process_record_embed_with_media_embed(
    embed_dict: dict,
) -> RecordWithMediaContextModel:  # noqa
    ...
    return RecordWithMediaContextModel(
        images_context=images_context, embedded_record_context=embedded_record_context
    )
...
def define_thread_context(post: dict) -> ThreadContextModel:
    ...
    parent_reply_context: ThreadPostContextModel = get_parent_reply_context(
        reply_parent
    )
    ...
    return ThreadContextModel(
        thread_root_post=parent_root_context, thread_parent_post=parent_reply_context
    )
...
def context_embed_url(post: dict) -> ContextEmbedUrlModel:
    ...
    is_link_to_trustworthy_news_article = url_is_to_news_domain(url) if url else False
    return ContextEmbedUrlModel(
        url=url, is_trustworthy_news_article=is_link_to_trustworthy_news_article
    )
...
def context_url_in_text(post: dict) -> ContextUrlInTextModel:
    ...
    return ContextUrlInTextModel(
        has_trustworthy_news_links=any([url_is_to_news_domain(link) for link in links])
    )
...
def post_linked_urls(post: dict) -> PostLinkedUrlsContextModel:
    ...
    return PostLinkedUrlsContextModel(
        url_in_text_context=url_in_text_context, embed_url_context=embed_url_context
    )
...
def post_author_context(post: dict) -> AuthorContextModel:
    ...
    return AuthorContextModel(
        post_author_is_reputable_news_org=(post["author"] in bsky_did_to_news_org_name)
    )
