...
def postprocess_feeds(feeds: list[UserFeedModel]) -> list[UserFeedModel]:
    """Postprocesses a feed for a given user."""
    return [postprocess_feed(user_feed=user_feed) for user_feed in feeds]


def convert_feeds_to_bluesky_format(
    feeds: list[UserFeedModel],
) -> list[CreatedFeedModel]:  # noqa
