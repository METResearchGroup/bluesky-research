...
def create_firehose_based_feeds(
    ...
    if len(reverse_chronological_users) > 0:
        posts: list[RecordClassificationMetadataModel] = load_firehose_based_posts()
...
def create_most_liked_feeds(
    ...
    representative_diversification_users = condition_to_user_map[
...
def create_feeds_per_condition(
    ...
    firehose_feeds: list[UserFeedModel] = create_firehose_based_feeds(
        condition_to_user_map
    )  # noqa
    output_feeds.extend(firehose_feeds)

    most_liked_feeds: list[UserFeedModel] = create_most_liked_feeds(
        condition_to_user_map
    )  # noqa
