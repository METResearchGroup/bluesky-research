...
class TestClassifyLatestPosts:
    ...
    def mock_write_posts_to_cache(self):
        ...
        with patch("ml_tooling.perspective_api.model.write_posts_to_cache") as mock:
            yield mock
    ...
    def test_classify_latest_posts_no_posts(
        self, mock_determine_backfill, mock_get_posts, mock_logger
    ):
    ...
    def test_classify_latest_posts_with_event(
        ...
        result = classify_latest_posts(
            backfill_period="days", backfill_duration=7, event=test_event
        )
    ...
    def test_classify_latest_posts_with_previous_metadata(
        ...
        """
        previous_metadata = {"latest_id": 123, "timestamp": "2024-01-01"}
    ...
    def test_passes_max_records_per_run_to_orchestrate_classification(self):
        ...
        with patch(
            "services.ml_inference.perspective_api.perspective_api.orchestrate_classification"
        ) as mock_orchestrate:
    ...
    def test_multi_step_classification_with_partial_success(
        ...
        result1 = classify_latest_posts(backfill_period="days", backfill_duration=7)
        ...
        result2 = classify_latest_posts(backfill_period="days", backfill_duration=7)
        ...
        result3 = classify_latest_posts(backfill_period="days", backfill_duration=7)
        ...
        # Verify write_posts_to_cache and return_failed_labels were called appropriately
        assert (
            mock_write_posts_to_cache.call_count == 3
        )  # Called for all successful posts
        assert (
            mock_return_failed_labels.call_count == 2
        )  # Not called in final run (no failures)
