...
class TestReturnFailedLabelsToInputQueue:
    ...
    def test_single_failed_label(self, mock_input_queue, mock_generate_datetime):
        ...
        failed_model = PerspectiveApiLabelsModel(
            uri="test_uri",
            text="test_text",
            reason="API_ERROR",
...
class TestWritePostsToCache:
    ...
    def test_empty_posts(self, mock_output_queue):
        """Test handling of empty posts list."""
        write_posts_to_cache([])
        mock_output_queue.batch_add_items_to_queue.assert_not_called()

    def test_single_post(self, mock_output_queue):
        """Test writing single post to cache."""
        post = PerspectiveApiLabelsModel(
            uri="test_uri",
            text="test_text",
            toxicity=0.8,
            severe_toxicity=0.3,
    ...
    def test_multiple_posts_with_batch(self, mock_output_queue):
        """Test writing multiple posts with batch size."""
        posts = [
            PerspectiveApiLabelsModel(
                uri=f"uri_{i}",
                text=f"text_{i}",
