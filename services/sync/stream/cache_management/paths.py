...
class CachePathManager:
    ...
    def _validate_operation(self, operation: Operation) -> dict:
        """Validate operation and return operation map."""
    ...
    def _validate_follow_path(
        self, operation: Operation, operation_map: dict, follow_status: FollowStatus
    ) -> str:
        """Validate follow path configuration and return relative path."""
    ...
    def _validate_non_follow_record_type(
        self, operation: Operation, operation_map: dict, record_type: RecordType
    ) -> str:
        """Validate non-follow record type and return relative path."""
    ...
    def get_study_user_activity_path(
        ...
        operation_map = self._validate_operation(operation)
        ...
        if record_type == RecordType.FOLLOW:
            ...
            relative = self._validate_follow_path(
                operation, operation_map, follow_status
            )
            return os.path.join(self.study_user_activity_root_local_path, relative)

        relative = self._validate_non_follow_record_type(
            operation, operation_map, record_type
        )
        return os.path.join(self.study_user_activity_root_local_path, relative)
    ...
    def get_relative_path(
        ...
        if record_type == RecordType.FOLLOW:
            ...
            return self._validate_follow_path(operation, operation_map, follow_status)

        return self._validate_non_follow_record_type(
            operation, operation_map, record_type
        )
