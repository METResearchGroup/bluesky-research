...
class SyncDataManager:
    """Container class that manages all export data for sync data."""
    ...
    def compress_cached_files_and_write_to_storage(
        ...
        s3_export_key = self.s3_export_key_map[operation][operation_type]
    ...
    def export_general_firehose_sync(
        ...
        for operation in ["create", "delete"]:
            for operation_type in self.operation_types:
                directory = self.export_filepath_map[operation][operation_type]
                for store in external_store:
                    self.compress_cached_files_and_write_to_storage(
                        directory=directory,
                        operation=operation,
    ...
    def export_study_user_post_local(
        self, base_path: str
    ...
    def export_study_user_follow_local(
        self, base_path: str
    ) -> tuple[list[dict], list[str]]:
    ...
    def export_study_user_like_local(
        self, base_path: str
    ) -> tuple[list[dict], list[str]]:
    ...
    def export_study_user_activity_local_data(self) -> list[str]:
        ...
        for operation in ["create", "delete"]:
            if operation == "create":
                ...
                for record_type in record_types:
                    if record_type == "post":
                        posts, filepaths = self.export_study_user_post_local(
                            base_path=create_path
                        )
                        all_posts.extend(posts)
                        all_filepaths.extend(filepaths)
                    elif record_type == "like":
                        likes, filepaths = self.export_study_user_like_local(
                            base_path=create_path
                        )
                        ...
                        replies_to_user_posts, filepaths = (
                            self.export_reply_to_study_user_post_local(
                                base_path=create_path
                            )
                        )  # noqa
        ...
        if len(all_follows) > 0:
            ...
            if any(
                [
                    follow["relationship_to_study_user"] == "follow"
    ...
    def export_in_network_user_activity_local_data(self) -> list[str]:
        ...
        author_dids: list[str] = os.listdir(
            os.path.join(self.root_write_path, key_root)
        )  # noqa
        ...
        if len(jsons) > 0:
            ...
            dtype_map = MAP_SERVICE_TO_METADATA["in_network_user_activity"][
                "dtypes_map"
            ]
    ...
    def export_batch(
        ...
        if clear_filepaths:
            ...
            if clear_cache:
                self.delete_cache_paths()
            self.rebuild_cache_paths()
    ...
    def export_study_user_post(
        ...
        the URI of the post.
        """
        relative_path = self.study_user_activity_relative_path_map[operation]["post"]
        root_path = self.study_user_activity_root_local_path
    ...
    def export_study_user_follow(
        ...
        relative_path = self.study_user_activity_relative_path_map[operation]["follow"][
            follow_status
        ]
        root_path = self.study_user_activity_root_local_path
        if not os.path.exists(root_path):
    ...
    def export_study_user_like(
        ...
        """
        relative_path = self.study_user_activity_relative_path_map[operation]["like"]
    ...
    def export_like_on_study_user_post(
        ...
        record.
        """
        relative_path = self.study_user_activity_relative_path_map[operation][
            "like_on_user_post"
        ]  # noqa
        post_uri_suffix = record["record"]["subject"]["uri"].split("/")[-1]
        record["record"] = json.dumps(record["record"])
    ...
    def export_study_user_data_local(
        ...
        if record_type == "post":
            ...
            self.export_like_on_study_user_post(
                record=record,
                operation=operation,
                author_did=author_did,
                filename=filename,
            ...
            self.export_reply_to_study_user_post(
                record=record,
                operation=operation,
                author_did=author_did,
                filename=filename,
    ...
    def export_in_network_user_post(self, record: dict, author_did: str, filename: str):
        """Exports a post record for an in-network user."""
        folder_path = os.path.join(
            self.in_network_user_activity_root_local_path, "create", "post", author_did
        )
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        full_path = os.path.join(folder_path, filename)
    ...
    def export_in_network_user_data_local(
        ...
        if record_type == "post":
            self.export_in_network_user_post(
...
if not os.path.exists("/app"):
    sync_data_manager = SyncDataManager()
    sync_data_manager.rebuild_cache_paths()
