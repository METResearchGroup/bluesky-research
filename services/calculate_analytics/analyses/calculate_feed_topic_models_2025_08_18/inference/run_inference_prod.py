#!/usr/bin/env python3
"""
Production Inference Runner for Topic Modeling

This script automatically finds the latest trained model in train/trained_models/prod/
and runs inference on production data using the inference.py script.

Usage:
    python run_inference_prod.py

The script will:
1. Find the latest model in train/trained_models/prod/
2. Find the corresponding metadata file
3. Run inference.py with the correct arguments for production mode
"""
...
def main():
    """Main function to run production inference"""
    logger.info("🔍 Production Inference Runner")
    logger.info("=" * 50)
    ...
    if success:
        ...
        logger.error("❌ Production inference failed!")
        sys.exit(1)
