#!/usr/bin/env python3
"""
Local UMAP Visualization Runner

This script automatically finds the latest local inference results and trained model,
then runs UMAP visualization on them. Designed for quick local testing and development.
...
def main():
    """Main function to run UMAP visualization with latest local results."""
    print("🔍 Local UMAP Visualization Runner")
    print("=" * 50)
    ...
    try:
        ...
        cmd = [
            sys.executable,
            "visualization/create_umap_visualization.py",
            "--model-path",
            model_path,
            "--metadata-path",
            metadata_path,
