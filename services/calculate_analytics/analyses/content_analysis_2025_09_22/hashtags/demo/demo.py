...
def demo_csv_export(hashtag_df):
    ...
    timestamp_dir = os.path.join(analysis_dir, timestamp)
    os.makedirs(timestamp_dir, exist_ok=True)

    # Export main results
    main_output_path = os.path.join(timestamp_dir, f"hashtag_analysis_{timestamp}.csv")
    ...
    overall_output_path = os.path.join(
        timestamp_dir, f"hashtag_overall_{timestamp}.csv"
    )
    ...
    for condition in hashtag_df["condition"].unique():
        ...
        condition_output_path = os.path.join(
            timestamp_dir, f"hashtag_condition_{condition}_{timestamp}.csv"
        )
    ...
    for period in hashtag_df["pre_post_election"].unique():
        ...
        period_output_path = os.path.join(
            timestamp_dir, f"hashtag_period_{period}_{timestamp}.csv"
        )
    ...
    metadata = {
        ...
        "total_hashtags": len(hashtag_df["hashtag"].unique()),
        "total_records": len(hashtag_df),
        "conditions": list(hashtag_df["condition"].unique()),
        "election_periods": list(hashtag_df["pre_post_election"].unique()),
    }

    import json

    metadata_path = os.path.join(
        timestamp_dir, f"hashtag_analysis_metadata_{timestamp}.json"
    )
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)
    print(f"   ✅ Metadata exported to: {os.path.basename(metadata_path)}")
...
def main():
    ...
    try:
        ...
        # Test CSV export
        analysis_dir = demo_csv_export(hashtag_df)
        ...
        print("\n📁 Check the 'results/analysis' folder for generated files")
