"""Downstream visualization script for hashtag analysis.

This script automatically finds the latest hashtag analysis results and creates
visualizations with different top K values. Given a CSV file with top N hashtags,
it can create visualizations showing top K hashtags where K < N.
...
def find_latest_results() -> str:
    """Find the latest hashtag analysis results directory."""
...
def load_hashtags_from_csv(csv_file: str) -> Dict[str, int]:
    """Load hashtags and counts from CSV file."""
    hashtags = {}
...
def create_top_k_visualization(
    ...
    """Create horizontal bar chart for top K hashtags."""
    # Sort hashtags by count (descending) and take top K
    sorted_hashtags = sorted(hashtags.items(), key=lambda x: x[1], reverse=True)[:top_k]

    if not sorted_hashtags:
        print(f"No hashtags found for {title}")
        return

    hashtag_names = [f"#{name}" for name, _ in sorted_hashtags]
    hashtag_counts = [count for _, count in sorted_hashtags]

    # Create visualization
    fig, ax = plt.subplots(figsize=(12, 8))

    # Create horizontal bars
    ax.barh(range(len(hashtag_names)), hashtag_counts, color=color, alpha=0.8)

    # Set labels and title
    ax.set_yticks(range(len(hashtag_names)))
    ax.set_yticklabels(hashtag_names)
...
def create_comparison_visualization(
    pre_hashtags: Dict[str, int],
    post_hashtags: Dict[str, int],
    top_k: int,
    output_path: str,
):
    """Create side-by-side comparison of pre/post election hashtags."""
    # Get top K hashtags for each period
    pre_top_k = sorted(pre_hashtags.items(), key=lambda x: x[1], reverse=True)[:top_k]
    post_top_k = sorted(post_hashtags.items(), key=lambda x: x[1], reverse=True)[:top_k]

    if not pre_top_k or not post_top_k:
        print("No hashtags found for comparison")
        return
    ...
    # Pre-election chart
    pre_names = [f"#{name}" for name, _ in pre_top_k]
    ...
    ax1.set_title(f"Top {top_k} Hashtags - Pre Election")
    ...
    # Post-election chart
    post_names = [f"#{name}" for name, _ in post_top_k]
    ...
    ax2.set_title(f"Top {top_k} Hashtags - Post Election")
...
def create_condition_comparison(
    ...
    for condition_name, csv_file in condition_files:
        if os.path.exists(csv_file):
            condition_data[condition_name] = load_hashtags_from_csv(csv_file)
        else:
    ...
    for i, (condition, hashtags) in enumerate(condition_data.items()):
        # Get top K hashtags for this condition
        ...
        if top_hashtags:
            ...
            sorted_hashtags = sorted(top_hashtags, key=lambda x: x[1], reverse=True)
    ...
    ax.set_xlabel("Frequency")
    ax.set_title(f"Top {top_k} Hashtags by Condition")
    ax.grid(True, alpha=0.3, axis="x")
...
def main():
    parser = argparse.ArgumentParser(
        description="Create downstream hashtag visualizations"
    )
    parser.add_argument(
        "--csv_file",
    ...
    parser.add_argument(
        "--top_k", type=int, required=True, help="Number of top hashtags to show"
    )
    parser.add_argument(
    ...
    print("🔍 Finding latest hashtag analysis results...")
    ...
    if args.analysis_type == "single":
        ...
        hashtags = load_hashtags_from_csv(csv_file)

        if not hashtags:
            print(f"No hashtags found in {csv_file}")
            return 1
        ...
        if not args.title:
            ...
            args.title = (
                f"Top {args.top_k} Hashtags - {base_name.replace('_', ' ').title()}"
            )

        # Create output filename
        output_filename = f"top_{args.top_k}_hashtags_{os.path.basename(csv_file).replace('.csv', '')}.png"
        output_path = os.path.join(output_dir, output_filename)

        # Create visualization
        create_top_k_visualization(
            hashtags, args.top_k, args.title, output_path, args.color
        )
        ...
        create_comparison_visualization(
            pre_hashtags, post_hashtags, args.top_k, comparison_output
        )
