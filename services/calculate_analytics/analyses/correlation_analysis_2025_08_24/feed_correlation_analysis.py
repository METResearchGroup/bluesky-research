...
def find_latest_csv_files() -> Tuple[Optional[str], Optional[str]]:
    """
    Find the latest daily and weekly CSV files from the results directory.
    ...
    # Separate daily and weekly files
    daily_files = [f for f in csv_files if "daily" in os.path.basename(f).lower()]
    weekly_files = [f for f in csv_files if "week" in os.path.basename(f).lower()]
    ...
    logger.info(
        f"Found {len(daily_files)} daily files, {len(weekly_files)} weekly files"
    )
    if latest_daily:
        logger.info(f"Latest daily file: {os.path.basename(latest_daily)}")
    if latest_weekly:
        logger.info(f"Latest weekly file: {os.path.basename(latest_weekly)}")

    return latest_daily, latest_weekly
