...
labels_filtered AS (
  ...
  FROM archive_ml_inference_intergroup l
  WHERE l.uri IN (SELECT uri FROM distinct_uris)
    AND l.label IS NOT NULL
    AND l.label != -1 -- Our default null label used during inference is -1
),
...
-- Final output: one row per feed with avg(label) over its posts.
SELECT
  p.feed_id,
  p.bluesky_handle,
  p.condition,
  ...
  avg(
    CASE
      -- IMPORTANT: for a given feed, some posts are unlabeled. We must NOT count those
      -- nulls toward the denominator of the per-feed proportion. AVG() ignores NULL.
      WHEN l.label IS NULL THEN NULL
      WHEN l.label = 1 THEN 1.0
      WHEN l.label = 0 THEN 0.0
      ELSE NULL
    END
  ) AS prop_intergroup_labeled,
  count(*) AS n_posts_in_feed,
  count(l.label) AS n_labeled_posts_in_feed
FROM feed_posts p
