...
| `compact_all_services` | Walks configured service datasets and compacts storage (for example merging partition files) across the stack as part of storage hygiene. | `services/compact_all_services/local_compaction.py` (scheduled via `pipelines/compact_all_services/handler.py`) |
