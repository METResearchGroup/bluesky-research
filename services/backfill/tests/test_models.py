...
class TestPostToEnqueueModel:
    """Tests for PostToEnqueueModel."""

    def test_creates_model_with_valid_fields(self):
        """Test that PostToEnqueueModel can be created with valid fields."""
        # Arrange
        uri = "test_uri"
        text = "test_text"
        ...
        # Act
        model = PostToEnqueueModel(
            uri=uri, text=text, preprocessing_timestamp=preprocessing_timestamp
        )

        # Assert
        assert model.uri == uri
        assert model.text == text
        assert model.preprocessing_timestamp == preprocessing_timestamp
    ...
    def test_model_dump_returns_dict(self):
        ...
        assert result["preprocessing_timestamp"] == "2024-01-01T00:00:00"


class TestPostUsedInFeedModel:
    """Tests for PostUsedInFeedModel."""

    def test_creates_model_with_valid_uri(self):
        """Test that PostUsedInFeedModel can be created with valid URI."""
        # Arrange
        uri = "test_uri"

        # Act
        model = PostUsedInFeedModel(uri=uri)
...
class TestPostScope:
    """Tests for PostScope enum."""

    def test_all_posts_enum_value(self):
        """Test that ALL_POSTS enum has correct value."""
        # Arrange & Act
        scope = PostScope.ALL_POSTS

        # Assert
        assert scope.value == "all_posts"

    def test_feed_posts_enum_value(self):
        """Test that FEED_POSTS enum has correct value."""
        # Arrange & Act
        scope = PostScope.FEED_POSTS

        # Assert
        assert scope.value == "feed_posts"

    def test_can_create_from_string_value(self):
        """Test that PostScope can be created from string value."""
        # Arrange
        value = "all_posts"

        # Act
        scope = PostScope(value)

        # Assert
        assert scope == PostScope.ALL_POSTS
