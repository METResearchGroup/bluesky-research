...
def flush_queues(pds_endpoint: str):
    """Flushes both the results and deadletter queues to permanent storage."""
    results_db_name = f"results_{pds_endpoint}_{start_timestamp}.db"
    deadletter_db_name = f"deadletter_{pds_endpoint}_{start_timestamp}.db"
    ...
    while not deadletter_queue.empty():
        deadletter_db.execute(
            "INSERT INTO deadletter (did, content) VALUES (?, ?)",
            deadletter_queue.get(),
        )
...
def run_backfill_for_pds_endpoint(pds_endpoint: str, dids: list[str]):
    ...
    add_dids_to_queue(dids=dids)
    logger.info(f"Added {len(dids)} DIDs to input queue.")
    total_processed_dids_counter = 0
    request_count = 0
    while not input_queue.empty():
        did = input_queue.get()
        if total_processed_dids_counter % 100 == 0:
            logger.info(f"Processing DID {did} of {len(dids)}")
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 429:
                ...
                time_start = time.time()
                flush_queues(pds_endpoint=pds_endpoint)
                time_end = time.time()
                logger.info(
    ...
    flush_queues(pds_endpoint=pds_endpoint)
    logger.info(f"Finished processing {len(dids)} DIDs.")
