...
class TransformedRecordModel(BaseModel):
    ...
    embed: Optional[ProcessedEmbed] = Field(
    ...
    py_type: Optional[te.Literal["app.bsky.feed.post", None]] = Field(
        default="app.bsky.feed.post", frozen=True
    )  # noqa


class PostMetadataModel(BaseModel):
    url: str = Field(..., description="The URL of the post.")
    source_feed: Optional[str] = Field(
        default=None, description="The source feed of the post."
    )  # noqa
    ...
    def validate_synctimestamp(cls, v):
        if not re.match(r"^\d{4}-\d{2}-\d{2}-\d{2}:\d{2}:\d{2}$", v):
            raise ValueError(
                "synctimestamp must be in 'YYYY-MM-DD-HH:MM:SS' format (e.g., '2024-04-23-04:41:17')"
            )  # noqa
        return v
...
class TransformedFeedViewPostModel(BaseModel):
    ...
    cid: str = Field(..., description="The CID of the post.")
    metadata: PostMetadataModel = Field(..., description="The metadata of the post.")  # noqa
    author: TransformedProfileViewBasicModel = Field(
